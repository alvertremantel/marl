...
pub fn write_run_meta(out: &OutputConfig) -> std::io::Result<()> {
    use marl_format::CELL_RECORD_STRIDE;
    ...
    let cell_record_stride = CELL_RECORD_STRIDE;
    ...
    writeln!(writer, "  \"endianness\": \"{}\",", ENDIANNESS)?;
    writeln!(writer, "  \"field_dtype\": \"{}\",", FIELD_DTYPE)?;
    writeln!(writer, "  \"field_layout\": \"{}\",", FIELD_LAYOUT)?;
...
mod tests {
    use super::*;
    use marl_format::ViewerCellRecord;
